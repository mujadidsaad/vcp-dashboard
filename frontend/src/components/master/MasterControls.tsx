/**
 * MasterControls — scan bar mirroring VCP's ScanControls layout.
 *
 * All filter/config controls live in the shared ScreenerSidebar.
 * This component renders:
 *   - Big red "Start Scan" / "Stop Scan" button
 *   - "Download CSV" outline button
 *   - Huge "N / N SCANNED" progress counter
 *   - Right-side stat trio: Matches, Errors, Complete %
 *   - Live-scan ribbon (currently scanning symbol)
 *   - Slim progress bar
 *   - Optional benchmark ribbon (Master-specific)
 *   - Cached-scan ribbon (with Rescan / Clear)
 */

import type { MasterResult, MasterVerdict } from '../../types';
...
interface Props {
  ...
  /** Kept so caller doesn't need to change; not visually rendered here. */
      ...
      <div className="flex items-center justify-between gap-4 flex-wrap">
        <div className="flex items-center gap-5 flex-wrap">
          ...
          <div className="flex items-baseline gap-1">
            <div className="stat-num text-4xl font-bold text-white leading-none">{p.processed}</div>
            <div className="stat-num text-lg text-white/40 leading-none">/ {p.total}</div>
